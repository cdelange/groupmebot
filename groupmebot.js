...
function formatObj(yahooObj, command) {
  ...
  switch (command) {
    ...
    case "@pointsagainst":
      ...
      postMessage(`Week ${currentWeek} Points Against Standings: \n` + sortedPointsAgainst);
      break;

    case "@pointsfor":
      let sortedPointsFor = standings
        .sort((a,b) => {
          return parseFloat(b.standings.points_for) - parseFloat(a.standings.points_for);
        }).map((team) => {
        return (
          standings.indexOf(team) + 1 +
          ". " +
      ...
      postMessage(`Week ${currentWeek} Points For Standings: \n` + sortedPointsFor);
      break;
    ...
    case "@pointsdiff":
      let sortedPointsDiff = standings
        ...
        return (
          ...
          " " +
          "(" +
          (team.standings.points_for-team.standings.points_against).toFixed(2) +
          ")"
        );
      }).join("\n");
      postMessage(`Week ${currentWeek} Points Differential Standings: \n` + sortedPointsDiff);
      break;
  }
}
